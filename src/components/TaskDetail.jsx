...
function TaskDetail({ task, onDelete, onToggleComplete, onUpdateDescription, onUpdateSubtasks }) { 
    ...
    return (
        <div className="task-detail">
            ...
            <div style={{ width: '100%', boxSizing: 'border-box' }}>
                ...
                <h1 style={{ wordBreak: 'break-word', whiteSpace: 'normal', margin: 0, padding: 0 }}>
                    {task.title}
                </h1>
            ...
            <div style={{ width: '100%' }}> 
                ...
                {isEditing ? (
                    ...
                    <div className="description-editor" style={{ width: '100%' }}> 
                        <textarea
                            value={currentDescription}
                            onChange={(e) => setCurrentDescription(e.target.value)}
                            rows="6"
                            style={{ width: '100%', padding: '10px', boxSizing: 'border-box', border: '1px solid #007bff' }}
                        />
                        <button onClick={handleSaveDescription} style={{ padding: '8px 15px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', marginRight: '10px' }}>
                            Kaydet
                        </button>
                        <button onClick={() => setIsEditing(false)} style={{ padding: '8px 15px', backgroundColor: '#6c757d', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
                            İptal
                        </button>
                    </div>
                ) : (
                    ...
                    <div 
                        ...
                        style={{ width: '100%', backgroundColor: '#f0f0f0', padding: '15px', borderRadius: '5px', minHeight: '80px', cursor: 'pointer' }}
                        onClick={() => setIsEditing(true)} 
                    >
                        <p>{task.description || "Açıklama eklemek için buraya tıklayın..."}</p>
                    </div>
                )}
                ...
                <form onSubmit={handleAddSubtask} style={{ marginTop: '15px', display: 'flex', gap: '10px' }}>
                    <input
                        type="text"
